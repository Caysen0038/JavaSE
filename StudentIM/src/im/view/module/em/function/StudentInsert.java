...
public class StudentInsert extends IMButton{
	/**
	 * 插入学生信息
	 */
	...
	private static StudentInsert function=new StudentInsert();
	private boolean inLoad=false;
	private StudentInsert(){
		/**
		 * 采用单例模式，不需要通过构造实例化，所以设为private
		 */
	}
	private StudentInsert(EM em){
		setEM(em);
	...
	public void init(){
		...
		this.add(Img.getImg("sources/img/studentInsert.png", 0
	...
	private void setRecord(){
		List<String> info=Config.getDOM().getField("student");
		Map<String,String> mean=Config.getDOM().getFieldName("student");
		...
		record.setTable("student");
	}
	private class StudentInfo extends Module{
		/**
		 * 显示刚插入的学生信息
		 */
