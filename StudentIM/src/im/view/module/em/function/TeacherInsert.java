...
public class TeacherInsert extends IMButton{
	/**
	 * 新增教师信息
	 */
	...
	private static TeacherInsert function=new TeacherInsert();
	private boolean inLoad=false;
	private TeacherInsert(){
		/**
		 * 采用单例模式，不需要通过构造实例化，所以设为private
		 */
	}
	private TeacherInsert(EM em){
		setEM(em);
	...
	public void init(){
		...
		this.add(Img.getImg("sources/img/teacherInsert.png", 0, 0,EM.ICON_WIDTH, EM.ICON_HEIGHT));
	...
	private void setRecord(){
		List<String> info=Config.getDOM().getField("teacher");
		Map<String,String> mean=Config.getDOM().getFieldName("teacher");
