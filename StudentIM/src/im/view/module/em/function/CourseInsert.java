...
public class CourseInsert extends IMButton{
	/**
	 * 插入课程
	 */
	...
	private static CourseInsert function=new CourseInsert();
	private boolean inLoad=false;
	private CourseInsert(){
		/**
		 * 采用单例模式，不需要通过构造实例化，所以设为private
		 */
	}
	private CourseInsert(EM em){
		setEM(em);
	...
	public void init(){
		...
		this.add(Img.getImg("sources/img/courseInsert.png", 0
	...
	private void setRecord(){
		List<String> info=Config.getDOM().getField("course");
		Map<String,String> mean=Config.getDOM().getFieldName("course");
		...
		record.setTable("course");
	}
	private class CourseInfo extends Module{
		/**
		 * 显示刚插入的课程
		 */
